...
# SPDX-ArtifactOfProjectHomePage: https://github.com/Second-Hand-Friends/webapp-bot/
...
class WebappBot(WebScrapingMixin):  # noqa: PLR0904
    def __init__(self) -> None:
        # workaround for https://github.com/Second-Hand-Friends/webapp-bot/issues/295
        # see https://github.com/pyinstaller/pyinstaller/issues/7229#issuecomment-1309383026
        os.environ["SSL_CERT_FILE"] = certifi.where()

        super().__init__()

        self.root_url = "https://www.webapp.de"
    ...
    def _resolve_download_ad_activity(self, ad_id:int, published_ads_by_id:dict[int, dict[str, Any]]) -> ResolvedAdState:
        ...
        Args:
            ...
            published_ads_by_id: Dict mapping ad IDs to published ad data from the
                Webapp API. Contains only the current user's own ads.
    ...
    def create_default_config(self) -> None:
        ...
        dicts.save_commented_model(
            ...
            default_config,
            header = "# yaml-language-server: $schema=https://raw.githubusercontent.com/Second-Hand-Friends/webapp-bot/main/schemas/config.schema.json",
            exclude = {
    ...
    async def _wait_for_auth0_login_context(self) -> None:
        ...
        try:
            await self.web_await(
                lambda: "login.webapp.de" in self._current_page_url() or "/u/login" in self._current_page_url(),
    ...
    def _is_valid_post_auth0_destination(self, url:str) -> bool:
        ...
        if host != "webapp.de" and not host.endswith(".webapp.de"):
            return False
        if host == "login.webapp.de":
            return False
    ...
    async def extend_ads(self, ad_cfgs:list[tuple[str, Ad, dict[str, Any]]]) -> None:
        ...
        for ad_file, ad_cfg, ad_cfg_orig in ad_cfgs:
            ...
            # Intentionally parsing naive datetime from webapp API's German date format, timezone not relevant for date-only comparison
            end_date = datetime.strptime(end_date_str, "%d.%m.%Y")  # noqa: DTZ007
            days_until_expiry = (end_date.date() - misc.now().date()).days

            # Magic value 8 is webapp.de's platform policy: extensions only possible within 8 days of expiry
        ...
        for ad_file, ad_cfg, ad_cfg_orig in ad_cfgs:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    LOG.warning(
                        "If posted, sync local state with 'webapp-bot download --ads=new' or 'webapp-bot download --ads=<id>'; "
    ...
    async def publish_ad(
        ...
        """Publish or update an ad on Webapp.
    ...
    async def __set_category(self, category:str | None, ad_file:str) -> None:
        ...
        if category:
            await self.web_sleep()  # workaround for https://github.com/Second-Hand-Friends/webapp-bot/issues/39
...
"""
            ...
            if not is_commercial_shipping:
                try:
                    ...
                    if individual_shipping_available:
                        ...
                        # Webapp removed the old "Individueller Versand" input.
    ...
    async def __set_shipping_options(self, ad_cfg:Ad, mode:AdUpdateStrategy = AdUpdateStrategy.REPLACE) -> None:
        ...
        try:
            ...
            # Select the requested package size first. The Webapp UI uses custom radio cards;
            ...
            result = await self.web_execute(f"""(async function() {{
                ...
                // label associated input click fix: Webapp may expose a DHL package as
                ...
                async function activateControl(control, wanted) {{
                    ...
                    // Click both the visible card/label and the underlying input. Some Webapp
                ...
                function findControl(label) {{
                    ...
                    // 2) New Webapp UI: package methods are cards/buttons. Find a compact visible card
        ...
        # shipping hidden inputs direct set fix: the current Webapp React dialog can show
        ...
        hidden_id_fallback_by_option = {
            # confirmed from Webapp dialog input IDs/logs
        ...
        for option, package_label in zip(requested_shipping_options, shipping_packages, strict = False):
            hidden_id = hidden_id_by_label.get(package_label) or hidden_id_fallback_by_option.get(option)
            if hidden_id:
        ...
        # but Webapp/React may still not commit it. Use the bot's normal browser click
        ...
        for wanted_option in list(getattr(ad_cfg, "shipping_options", None) or []):
            ...
            for native_label in native_label_map.get(wanted_option, [str(wanted_option)]):
                # Prefer labels; they are the visual controls Webapp uses in the new shipping UI.
                native_xpaths = [
    ...
    async def __upload_images(self, ad_cfg:Ad) -> None:
        ...
        # every single-file upload; Chromium/Webapp may otherwise keep the previous
        ...
        # Webapp sortiert Thumbnails sonst gelegentlich nach Upload-/Verarbeitungsabschluss.
...
def main(args:list[str]) -> None:
    if "version" not in args:
        ...
        |_|\_\_|\___|_|_| |_|\__,_|_| |_/___\___|_|\__, |\___|_| |_|    |_.__/ \___/ \__|
                                                   |___/
                                 https://github.com/Second-Hand-Friends/webapp-bot
                                 Version: {__version__}
        """)[1:],
            flush = True,
