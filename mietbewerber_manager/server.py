...
APP_VERSION = "0.2.20"
PUBLIC_BASE_URL = "https://mieter.primary-digital.de"
...
USER_NAMES = ["admin", "secondary", "user", "Carsten", "Pascal", "Celina"]
...
def can_edit_applicant_fields(user: dict[str, object]) -> bool:
    """Allow full admins and secondary to correct structured applicant fields."""
    return user.get("role") == "admin" or str(user.get("username", "")).casefold() == "secondary"
        ...
        users.append(
            {
                ...
                "display_name": "primary" if username == "admin" else username,
                "created_at": now(),
                "last_login_at": None,
...
def migrate_db(data: dict[str, object]) -> bool:
    ...
    for user in users:
        ...
        if "display_name" not in user:
            user["display_name"] = "primary" if str(user.get("username", "")).casefold() == "admin" else user.get("username", "")
            changed = True
...
def redact_for_gemini(text: str) -> tuple[str, int]:
    ...
    # primary 49" or "Miriam (40) & Jonas (42)". Both names remain personal
...
function renderDetail(){
  ...
  const canEditFields=admin||String(user.username||'').toLowerCase()==='secondary';
...
function renderDetailV020(){
  ...
  const user=state.current_user,admin=user.role==='admin',canEdit=admin||String(user.username).toLowerCase()==='secondary';reviewFieldsDraft=[...(item.analysis_review_fields||[])];
