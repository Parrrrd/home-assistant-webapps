...
test("local CalDAV reuses the established exclamation-mark notification group", () => {
  const { reminderNotificationPayload } = require("./notification-routing");
  const payload = reminderNotificationPayload("Es wurde Milch zur Einkaufsliste hinzugefügt!", { source: "local-caldav" });
  assert.equal(payload.data.tag, "alexa-bring-sync");
  assert.equal(payload.data.group, "alexa-bring-sync");
});

test("local CalDAV sends the reliable exclamation-mark push directly and marks its entry source", () => {
  const serverSource = fs.readFileSync(path.join(__dirname, "server.js"), "utf8");
  assert.match(serverSource, /notificationSource: notificationContext\.source === "local-caldav" \? "local-caldav" : ""/);
  assert.match(serverSource, /if \(notificationContext\.source === "local-caldav"\) await notifyReminderImport\(name, notificationContext\)/);
  assert.match(serverSource, /`Es wurde \$\{name\} zur Einkaufsliste hinzugefügt!`/);
  assert.match(serverSource, /payload\.notificationSource === "local-caldav" \? \{ \.\.\.payload, status: "completed" \} : payload/);
});
