...
test("creates an exact pre-migration backup before loading an older data version", () => {
  ...
  const backups = fs.readdirSync(backupDir).filter((name) => /^pre-migration-0\.3\.13-to-0\.3\.63-.*\.json$/.test(name));
...
test("all release version declarations are synchronized", () => {
  ...
  assert.match(serverSource, /const VERSION = "0\.3\.63"/);
  assert.match(config, /^version: 0\.3\.63$/m);
  assert.match(dockerfile, /^ARG BUILD_VERSION=0\.3\.63$/m);
  assert.equal(pkg.version, "0.3.63");
  assert.match(sw, /shell-0\.3\.63/);
  assert.match(manifest, /\?v=0\.3\.63/);
});
...
test("Dockerfile uses the current generic Home Assistant multiarch base and packaged CalDAV dependencies", () => {
  ...
  assert.match(dockerfile, /apk add --no-cache apache2-utils nodejs radicale/);
...
test("service worker keeps image cache across releases and image requests stay network-first", () => {
  const sw=fs.readFileSync(path.join(__dirname,"assets","sw.js"),"utf8");
  assert.match(sw,/eigene-einkaufsliste-shell-0\.3\.63/);
...
test("service run scripts start the app and the separate CalDAV service without runtime installs", () => {
  ...
  assert.match(caldavRun,/exec radicale --config \/etc\/radicale\/config/);
...
test("0.3.51 starts migration with inherited product images before server initialization", () => {
  ...
  assert.equal(result.version, "0.3.63");
