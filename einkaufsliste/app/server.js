...
function friendlyProductName(value) {
  ...
  const frozenPrefix = /^(?:tk|gefroren(?:en|er|es|em|e)?|tiefgefroren(?:en|er|es|em|e)?|tiefgek(?:ü|ue)hlt(?:en|er|es|em|e)?)\s*[-–—]?\s*(.+)$/i.exec(text);
  if (frozenPrefix?.[1]) text = `TK-${firstUpper(frozenPrefix[1])}`;
...
function batteryVariantFromName(value) {
  const text = String(value || "").trim().replace(/\s+/g, " ");
  const match = /^(.*?)\b(batterie|batterien)\b(.*?)$/i.exec(text);
  if (!match) return null;
  const normalizeVariant = (part) => {
    const compact = String(part || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
    return /^(?:A{2,4}|C|D|9V|CR\d{4}|LR\d{1,4})$/.test(compact) ? compact : "";
  };
  const before = String(match[1] || "").trim();
  const after = String(match[3] || "").trim();
  const variant = !after ? normalizeVariant(before) : (!before ? normalizeVariant(after) : "");
  if (!variant) return null;
  return { name: /^batterien$/i.test(match[2]) ? "Batterien" : "Batterie", variant };
}

function prepareParsedProductDetail(parsed) {
  if (!parsed) return parsed;
  let next = { ...parsed, name: friendlyProductName(parsed.name) };
  const batteryVariant = batteryVariantFromName(next.name);
  if (batteryVariant) {
    next.name = batteryVariant.name;
    if (!next.productDetail) next.productDetail = batteryVariant.variant;
  }
...
function isExplicitTargetCategoryId(list, categoryId) {
  const category = (list?.categories || []).find((item) => item.id === categoryId);
  if (!category) return false;
  return isFirmaCategoryName(category.name) || isReweCategoryName(category.name) || isRetailerCategoryName(category.name) || isMeyerhofCategoryName(category.name);
}
...
function recordLearningExampleForList(list, productName, fromCategoryId, toCategoryId) {
  if (!list || !productName || !toCategoryId || fromCategoryId === toCategoryId || isExplicitTargetCategoryId(list, toCategoryId)) return;
...
function learnedCategoryForName(itemName, list) {
  ...
  const exact = examples
    .filter((example) => example.normalized === normalizedItem && categoryExistsForList(list, example.toCategoryId) && !isExplicitTargetCategoryId(list, example.toCategoryId))
  ...
  for (const [key, points] of score.entries()) {
    ...
    if (points < requiredPoints || (conflicts.get(term)?.size || 0) > 1 || !categoryExistsForList(list, categoryId) || isExplicitTargetCategoryId(list, categoryId)) continue;
...
function classificationExamplesText(list) {
  const examples = (list?.learningExamples || []).filter((example) => !isExplicitTargetCategoryId(list, example.toCategoryId)).slice(-25);
...
function normalProductForName(name, list) {
  const key = normalizeText(name);
  const compact = compactProductKey(name);
  if (!key) return null;
  let matches = (state.products || []).filter((product) => productSearchKeys(product).includes(key));
  if (!matches.length && compact.length >= 5) matches = (state.products || []).filter((product) => productSearchKeys(product).some((candidate) => compactProductKey(candidate) === compact));
  matches = matches.filter((product) => !isExplicitTargetCategoryId(list, product.categoryId));
  const ordinary = matches.filter((product) => !product.sourceTag);
  if (ordinary.length === 1) return ordinary[0];
  if (!ordinary.length && matches.length === 1) return matches[0];
  return null;
}

function existingVariantForInheritance(name) {
  const key = normalizeText(name);
  const compact = compactProductKey(name);
  if (!key) return null;
  let matches = (state.products || []).filter((product) => productSearchKeys(product).includes(key));
  if (!matches.length && compact.length >= 5) matches = (state.products || []).filter((product) => productSearchKeys(product).some((candidate) => compactProductKey(candidate) === compact));
  if (!matches.length) return null;
  return matches.find((product) => iconEditState(product) === "processed") || matches[0];
}
...
function productForParsed(parsed, options = {}) {
  ...
  const catalogRule = options.forcedCategoryId ? null : catalogMatchForName(parsed.name);
  const catalogCategoryId = catalogRule && state.categories.some((category) => category.id === catalogRule.categoryId) ? catalogRule.categoryId : "";
  const categoryId = options.forcedCategoryId || hard?.categoryId || learned?.categoryId || usableRule?.categoryId || catalogCategoryId || "other";

  // Normale Eingaben werden immer zuerst in ihrer normalen Produktkategorie gesucht.
  // Firma/REWE/DM-Rossmann-Müller/Meyerhof sind reine explizite Ziele und dürfen
  // niemals nur deshalb Standard werden, weil dort bereits der ungetaggte Altartikel liegt.
  if (!options.forcedCategoryId && !options.sourceTag) {
    const scopedOptions = { categoryId, strictCategory: true, allowTaggedCategoryMatch: true };
    const scoped = productForName(lookupName, scopedOptions) || productForName(parsed.name, scopedOptions);
    if (scoped && !isExplicitTargetCategoryId(list, scoped.categoryId)) return improveProduct(scoped);
    const normalExisting = normalProductForName(lookupName, list) || normalProductForName(parsed.name, list);
    if (normalExisting) return improveProduct(normalExisting);
  }
  ...
  if (!options.forcedCategoryId && !options.sourceTag) {
    const inheritedFrom = existingVariantForInheritance(lookupName) || existingVariantForInheritance(parsed.name);
    if (inheritedFrom) {
      created.classificationSource = "inherited";
      created.clonedFromProductId = inheritedFrom.id;
      created.aliases = [...new Set([...(inheritedFrom.aliases || []), ...created.aliases].map(normalizeText).filter(Boolean))];
      copyExistingProductMetadata(inheritedFrom, created);
    }
  }
  state.products.push(created);
  return created;
