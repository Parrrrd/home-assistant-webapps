...
prepare_candidate() {
  ...
  perl -0pi -e "s/version: 0\\.1\\.16/version: ${version}/" "$candidate/webapp_updater/config.yaml"
  perl -0pi -e "s/BUILD_VERSION=0\\.1\\.16/BUILD_VERSION=${version}/" "$candidate/webapp_updater/Dockerfile"
...
prepare_candidate "$zip_repository" 0.1.17 '' "$zip_candidate"
(cd "$zip_candidate" && zip -qr "$test_dir/webapp_updater-0.1.17.zip" webapp_updater)
sh "$zip_repository/scripts/import-google-drive-package.sh" "$test_dir/webapp_updater-0.1.17.zip" webapp_updater-0.1.17.zip
grep -Fqx 'version: 0.1.17' "$zip_repository/webapp_updater/config.yaml"
...
prepare_candidate "$bad_zip_repository" 0.1.17 ghcr.io/example/incorrect "$bad_zip_candidate"
(cd "$bad_zip_candidate" && zip -qr "$test_dir/webapp_updater-bad.zip" webapp_updater)
if sh "$bad_zip_repository/scripts/import-google-drive-package.sh" "$test_dir/webapp_updater-bad.zip" webapp_updater-0.1.17.zip; then
...
prepare_candidate "$patch_repository" 0.1.17 '' "$test_dir/patch-candidate"
rm -rf "$patch_repository/webapp_updater"
cp -a "$test_dir/patch-candidate/webapp_updater" "$patch_repository/"
(cd "$patch_repository" && git diff -- webapp_updater > "$test_dir/webapp_updater-0.1.17.patch" && git restore webapp_updater)
sh "$patch_repository/scripts/import-google-drive-patch.sh" "$test_dir/webapp_updater-0.1.17.patch" webapp_updater-0.1.17.patch
grep -Fqx 'version: 0.1.17' "$patch_repository/webapp_updater/config.yaml"
...
cat > "$test_dir/webapp_updater-0.1.17.patch" <<'PATCH'
...
+/etc/passwd
PATCH
if sh "$symlink_repository/scripts/import-google-drive-patch.sh" "$test_dir/webapp_updater-0.1.17.patch" webapp_updater-0.1.17.patch; then
