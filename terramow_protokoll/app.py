...
APP_NAME = "TerraMow Protokoll"
APP_VERSION = "0.1.9"
DATA_ROOT = Path(os.environ.get("DATA_ROOT", "/data")) / "terramow_protokoll"
...
DEFAULTS = {
    "camera_entity": "camera.terramow_karte",
    "mower_entity": "lawn_mower.terramow_mahroboter",
    "duration_entity": "sensor.terramow_dauer_der_aktuellen_sitzung",
    "mode_entity": "sensor.terramow_betriebsmodus",
    "retain_count": 10,
    "poll_seconds": 15,
    "end_debounce_seconds": 90,
    "min_auto_duration_seconds": 60,
    "active_states": "mowing",
    "end_states": "docked",
    "port": 8143,
    "min_auto_path_points": 10,
    "min_auto_green_ratio": 0.002,
}
...
AREA_LABEL_OPTIONS = [
    ...
    "Mähen Schaukel",
    "Mähen Pool",
    "Mähen Terrasse und Schaukel",
    "Mähen Terrasse und Pool",
    "Mähen Pool und Schaukel",
    "Mähen Terrasse, Schaukel und Pool",
    "Rand Terrasse",
    "Rand Schaukel",
    "Rand Pool",
    "Rand Terrasse und Schaukel",
    "Rand Terrasse und Pool",
    "Rand Pool und Schaukel",
    "Rand Terrasse, Schaukel und Pool",
]
...
def green_area_signature(image_bytes: Optional[bytes]) -> Optional[Dict[str, Any]]:
    ...
    The TerraMow base map is mostly stable; the changing green overlay marks
...
runtime_status: Dict[str, Any] = {
    ...
    "last_active_snapshot_at": None,
    "last_active_snapshot_error": None,
}
...
def end_states(cfg: Dict[str, Any]) -> List[str]:
    raw = str(cfg.get("end_states", ""))
    states = [x.strip().lower() for x in re.split(r"[,;\n]+", raw) if x.strip()]
    return states or ["docked"]
...
def mode_label(mode: Optional[str]) -> str:
    mapping = {
        "MAP_CLEAN_INFO_MODE_GLOBAL": "Auto",
        "MAP_CLEAN_INFO_MODE_SELECT_REGION": "Zone",
        "MAP_CLEAN_INFO_MODE_DRAW_REGION": "Bereich",
        "MAP_CLEAN_INFO_MODE_MOVE_TO_TARGET_POINT": "Zielpunkt",
    }
    if not mode:
        return "–"
...
def collect_states(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    duration = ha_get_state(str(cfg.get("duration_entity")))
...
def mowing_evidence_ok(
    ...
    """Verhindert automatische Protokolle ohne echten Mähvorgang.

    v0.1.5: nicht mehr beide Kriterien hart verlangen.
    Bei TerraMow kann die Karte nach dem Docken schon teilweise zurückgesetzt sein.
    ...
    if has_path or has_green:
        ...
        return True, "Mähspuren erkannt: " + ", ".join(details)

    if path_points is None and green_ratio is None:
        return False, "Keine auswertbaren Mähspuren in Kamera/Map gefunden"
    ...
    return False, "Zu wenig Mähspuren: " + ", ".join(reasons)
...
def save_session_entry(
    ...
    snapshot_content_override: Optional[bytes] = None,
) -> Dict[str, Any]:
    ...
    if snapshot_content_override:
        snapshot_content = snapshot_content_override
        try:
            image_path.write_bytes(snapshot_content_override)
        except Exception as e:
    ...
    entry = {
        ...
        "area_label": inferred_label,
        "area_source": "auto-image" if inferred_label else None,
...
class MowerMonitor:
    def __init__(self) -> None:
        ...
        self.session_snapshot_content: Optional[bytes] = None
        self.session_snapshot_camera_state: Optional[Dict[str, Any]] = None
        self.session_snapshot_at: Optional[datetime] = None
        self.session_snapshot_error: Optional[str] = None

    def reset_session_snapshot(self) -> None:
        self.session_snapshot_content = None
        self.session_snapshot_camera_state = None
        self.session_snapshot_at = None
        self.session_snapshot_error = None

    def refresh_active_snapshot(self, cfg: Dict[str, Any], camera_state: Optional[Dict[str, Any]], now: datetime) -> None:
        """Save the latest map while the mower is really mowing.

        v0.1.8: The previous version grabbed the map only after the mower had
        docked. On some TerraMow/HA setups the green mowing overlay is already
        gone by then, so the automatic evidence check skipped the whole entry.
        Keeping an in-memory snapshot during mowing fixes that without storing
        empty post-docking maps.
        """
        try:
            content, _ctype = camera_snapshot_bytes(cfg, camera_state=camera_state)
            if content:
                self.session_snapshot_content = content
                self.session_snapshot_camera_state = camera_state
                self.session_snapshot_at = now
                self.session_snapshot_error = None
                with lock:
                    runtime_status["last_active_snapshot_at"] = iso(now)
                    runtime_status["last_active_snapshot_error"] = None
        except Exception as e:
            self.session_snapshot_error = str(e)
            with lock:
                runtime_status["last_active_snapshot_error"] = str(e)
    ...
    def poll_once(self) -> None:
        ...
        ends = end_states(cfg)
        mower_state_lower = str(mower_state).lower() if mower_state is not None else ""
        is_active = (mower_state_lower in actives) if mower_state is not None else False
        is_end_state = (mower_state_lower in ends) if mower_state is not None else False
        ...
        if self.current_session is None:
            ...
            if not recent_duplicate and (is_active or duration_increasing):
                ...
                if is_active or duration_increasing:
                    self.refresh_active_snapshot(cfg, camera_state, now)
        else:
            ...
            if is_active or duration_increasing:
                ...
                # Keep the latest useful map from the active mowing phase.
                # Limit refreshes slightly so HA is not hammered on long runs.
                if self.session_snapshot_at is None or (now - self.session_snapshot_at).total_seconds() >= 30:
                    self.refresh_active_snapshot(cfg, camera_state, now)
            else:
                # v0.1.8: Only finish the automatic log when the mower is actually docked
                # (or in another configured end state). Intermediate states such as returning
                # no longer trigger a premature post-run snapshot.
                if is_end_state:
                    ...
                    if (now - candidate).total_seconds() >= int(cfg.get("end_debounce_seconds", 90)):
                        ...
                        saved = save_session_entry(
                            ...
                            camera_state=self.session_snapshot_camera_state or camera_state,
                            source="automatic-mowing-docked",
                            # v0.1.9: Wenn wirklich mowing -> docked erkannt wurde,
                            # ist das bereits der Nachweis für einen echten Mähvorgang.
                            # Der zusätzliche Bild-/Pfadfilter war zu streng und konnte
                            # den kompletten Protokolleintrag verwerfen.
                            require_mowing_evidence=False,
                            snapshot_content_override=self.session_snapshot_content,
                        )
        ...
        # Fallback: Some TerraMow/HA versions only update the duration sensor at the end
...
def render_index() -> str:
    ...
    history_html = "\n".join(cards) if cards else '<section class="card small">Noch kein gespeicherter Mähvorgang vorhanden.</section>'
    body = f"""
    <h2>Letzte Mähvorgänge</h2>
...
"""
    return html_page("TerraMow Live-Karte", body, script)
...
def settings() -> Any:
    ...
    labels = {
        ...
        "duration_entity": "Dauer-Sensor",
        "mode_entity": "Modus-Sensor",
        ...
        "active_states": "Start-Zustand",
        "end_states": "End-Zustand",
        "port": "Port",
    }
    for key in DEFAULTS:
        rows.append(f"""
    ...
    <form method="post" class="card">
      {''.join(rows)}
      <p class="small">Aktive Zustände sind komma-getrennt. Bei direktem Start/Ende über Start-Zustand → End-Zustand wird gespeichert. Der Mähspuren-Filter gilt nur noch für den Dauer-Fallback.</p>
...
"""
    return html_page("TerraMow Einstellungen", body, script)


@app.route("/health")
def health() -> Response:
    return jsonify({"status": "ok", "version": APP_VERSION, "feature": "mowing-docked-ohne-evidence-filter"})
...
def live_jpg() -> Any:
    ...
    try:
        ...
        svg = f"""<svg xmlns='http://www.w3.org/2000/svg' width='900' height='500'><rect width='100%' height='100%' fill='#111'/><text x='30' y='60' fill='#fff' font-family='Arial' font-size='26'>TerraMow Live-Karte nicht verfügbar</text><text x='30' y='110' fill='#aaa' font-family='Arial' font-size='18'>{str(e)[:180]}</text></svg>"""
...
def start_monitor() -> None:
    ...
    if monitor_thread and monitor_thread.is_alive():
        return
    monitor_thread = threading.Thread(target=monitor.run, name="terramow-monitor", daemon=True)
